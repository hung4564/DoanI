...
$enrollment_link=route('dashboard::courses.enrollment');
$_listLink = route($resourceRoutesAlias.'.listCourses');
$tableCounter = 0;
$total = 0;
if (count($records) > 0) {
    $total = $records->total();
    ...
    <div class="box box-info">
            ...
            <div class="box-tools pull-right">
                <form class="form" role="form" method="GET" action="{{ $_listLink }}">
                    <div class="input-group input-group-sm margin-r-5 pull-left" style="width: 200px;">
                     ...
                     <a href="{{$enrollment_link}}" class="btn btn-sm btn-primary pull-right">
        ...
        <div class="box-body no-padding">
            @if (count($records) > 0)
                ...
                </div><div class="table-responsive list-records">
                    <table class="table table-hover table-bordered">
                        ...
                        @foreach ($records as $record)
                            ...
                            <!--<td><input type="checkbox" name="ids[]" value="{{ $record->id }}" class="square-blue"></td>-->
                                <td>{{ $tableCounter }}</td>
                                <td class="table-text">
                                  <a href="{{route('course.detail',[$record->id])}}">{{ $record->name }}</a>
                                </td>
                                <td>
                                  <a href="{{route('course.detail',[$record->id])}}" class="btn btn-info btn-sm"><i class="fa fa-list"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
