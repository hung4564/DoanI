...
<div class="col-md-6">
  ...
  <div class="form-group margin-b-5 margin-t-5{{ $errors->has('status') ? ' has-error' : '' }}">
    ...
    <?php ($status= isset($record->status)?$record->status:0);?>
    <div class="radio">
      <label>
        <input type="radio" name="status" class="minimal-red" @if($status==0 ) checked @endif value="0">
        Disabe
      </label>
      <label>
        <input type="radio" name="status" class="minimal-red"  @if($status==1 )checked @endif  value="1">
        Enable
      </label>
    </div>
  </div>
...
<div class="col-md-6">
  <div class="form-group margin-b-5 margin-t-5{{ $errors->has('course') ? ' has-error' : '' }}">
    ...
    <div class="Course">
      @if(isset($record->course_id))
        ...
        @else
        <a href="{{route('dashboard::courses.detail',['id' => $record->course_id])}}">
  ...
  <div class="form-group margin-b-5 margin-t-5{{ $errors->has('questions') ? ' has-error' : '' }}">
    ...
    <div class="question">
      @if(isset($record->id))
      <a href="{{route('dashboard::QuizQuestion.index',['id' => $record->id])}}">
      <button type="button" class="btn btn-info" onclick="">
